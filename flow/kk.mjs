...
console.log('secties:', await p.locator('body > section').count(), '| kisten:', await p.locator('.kisten .kist').count(), '| dossier weg:', await p.locator('.dossier').count() === 0);
await p.locator('.kisten').scrollIntoViewIfNeeded(); await p.waitForTimeout(500);
await p.screenshot({ path: process.argv[2] + '/kisten2.png' });
